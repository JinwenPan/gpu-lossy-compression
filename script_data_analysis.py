...
class Analysis:
    def __init__(self, data_folder, output_folder, data_dims, data_type='f32', data_type_size=4, cmp_list=None, eb_list=None, br_list=None,
        ...
        self.cmp_list = cmp_list or ['cuszhi']
            ...
            for data_point in self.datapoint_list[:-1]:
                ...
                try: 
                    ...
                    for line_number, line in enumerate(lines):
                        line_split = line.split()
                        # compression
                        if "(total)" in line and line_number > 0 and ("(c) COMPRESSION REPORT" in lines[line_number-6] or "(c) COMPRESSION REPORT" in lines[line_number-8]):
                        ...
                        if "(total)" in line and line_number > 0 and "(d) deCOMPRESSION REPORT" in lines[line_number-7]:
                            index = line_split.index("(total)") + 2
                    ...
                    self.analyze_nsys(lines[nsys_comp_line_number[0]:nsys_comp_line_number[1]], df, (eb, data_point), 
                          'nsys_cmp_cTP', self.data_size, ["psz::detail::hf_encode_phase2_deflate", "psz::detail::hf_encode_phase1_fill", "histsp_multiwarp", "psz::detail::hf_encode_phase4_concatenate", 
                                                           "cusz::c_spline_infprecis_data", "cusz::c_spline_profiling_data_2", 
                                                           "d_encode_rtr", "d_encode_tcms", "d_encode_bitr"])
                    self.analyze_nsys(lines[nsys_decomp_line_number[0]:nsys_decomp_line_number[1]], df, (eb, data_point), 
                          'nsys_cmp_xTP', self.data_size, ["hf_decode_kernel",
                                                           ...
                                                           "d_decode_rtr", "d_decode_tcms", "d_decode_bitr"])
